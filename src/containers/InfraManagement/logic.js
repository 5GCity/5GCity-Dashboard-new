...
export default kea({
  ...
  reducers: ({ actions }) => ({
    pinsResources: [
      ...
      PropTypes.array,
      {
        ...
        [actions.setListResources]: (state, payload) =>
          CreateAllPins(payload.resources),
        [actions.reset]: () => []
      }
    ],
    markerSelect: [
    ...
    linksResources: [
      ...
      PropTypes.array,
      {
        [actions.setChunketeTopology]: (state, payload) =>
          CreateAllLinks(payload.rans)
      }
    ]
  }),
  ...
  workers: {
    *fetchResources(action) {
      ...
      try {
        const [responseComputes, responseNetworks, responseRAN] = yield all([
